...
INT_PTR CALLBACK MainDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) 
	{
       ...
       case WM_COMMAND:
        ...
        case WM_ACTIVATE:
            if (wParam && g_pMouse != WA_INACTIVE)
			{
                // Make sure the device is acquired, if we are gaining focus.
                g_pMouse->Acquire();
            }
			if (wParam && g_pKeyboard != WA_INACTIVE)
			{
				g_pKeyboard->Acquire();
			}
            break;
...
HRESULT OnInitDialog(HWND hDlg)
{
	...
	keycommand = GetPrivateProfileInt("commands", "resetview", 88, ".\\tir2vjoy.ini");
    ...
    if (result == NP_OK)
	{
		if (tid.wNPStatus == NPSTATUS_REMOTEACTIVE) 
		{
			if (NPFrameSignature != tid.wPFrameSignature) 
			{
				...
				if (lstrcmp(strOldText, strNewText) != 0)
				{
...
HRESULT ReadImmediateMouseData(HWND hDlg)
{
    ...
    if (g_pMouse == NULL || g_pKeyboard == NULL)
	{
        return S_OK;
	}
